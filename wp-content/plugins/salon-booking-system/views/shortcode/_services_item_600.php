<label for="<?php echo SLN_Form::makeID('sln[services][' . $service->getId() . ']') ?>" class="row sln-service sln-service--<?php echo $service->getId(); ?>">
    <div class="col-xs-12 sln-service__header">
		...
		<?php if ($thumb): ?>
            ...
            <div class="col-xs-10 col-sm-4 col-xs-push-10- col-sm-push-3 <?php echo $service->isVariableDuration() ? 'sln-service-variable-duration--block' : ''?>">
...
<?php if($showPrices): ?>
            <h3 class="sln-steps-price sln-service-price">
                <?php if ($service->getVariablePriceEnabled()): ?>
                    <?php _e('from', 'salon-booking-system') ?>
                <?php endif; ?>
                <span class="sln-service-price-value"><?php echo $plugin->format()->moneyFormatted($service->getPrice())?></span>
...
<?php endif ?>
		...
		<?php else: ?>
            ...
            <div class="col-xs-10 col-sm-6 col-xs-push-10- col-sm-push-0 <?php echo $service->isVariableDuration() ? 'sln-service-variable-duration--block' : ''?>">
                <?php include 'service_variable_duration.php'; ?>
<?php if($showPrices): ?>
            <h3 class="sln-steps-price sln-service-price">
                <?php if ($service->getVariablePriceEnabled()): ?>
...
<?php endif ?>
		...
		<?php endif; ?>
        <?php if ($thumb): ?>
            <div class="col-xs-10 col-sm-3 col-xs-pull-10- col-sm-pull-8  col-sm-push-0 sln-steps-thumb sln-service-thumb">
                <?php echo $thumb ?>
            </div>
        <?php else: ?>
        <?php endif; ?>
            ...
            <div class="col-xs-12 col-sm-10 col-md-9 col-sm-offset-1">
                    ...
                    ?>
                        <span class="sln-steps-duration sln-service-duration">
                            <small><?php echo __('Duration', 'salon-booking-system')?>:</small> <span><?php echo $duration->format('H:i'); ?></span>
                        </span>
                    <?php endif ?>
                <!-- .sln-service-info // END -->
            </div>
