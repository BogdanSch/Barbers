...
(function($) {
	...
	Calendar.prototype._calculate_hour_minutes_daily = function(data) {
		...
		if ($self.options._assistants_mode) {
			...
			$.each(data.events, function(k, e) {
				...
				if (e._end < start.getTime()) {
					...
					time_start = ['' + start
							...
							'' + start
								...
								.toString()
								.formatNum(2)];
					$self.options.time_start = time_start.join(':');
					let time_diff = parseInt(time_end[0]) * 60 +
									parseInt(time_end[1]) -
									(parseInt(time_start[0]) * 60 + parseInt(time_start[1]));
				...
				if (e._start > end.getTime()) {
					...
					time_end = ['' + end
							...
							'' + end
								...
								.toString()
								.formatNum(2)];
					if(parseInt(time_end[0]) < parseInt(time_start[0])){
						time_end[0] = new String(parseInt(time_end[0]) + 24);
					}
					$self.options.time_end = time_end.join(':');
					let time_diff = parseInt(time_end[0]) * 60 +
									parseInt(time_end[1]) -
									(parseInt(time_start[0]) * 60 + parseInt(time_start[1]));
					data.lines = time_diff / time_split;
				...
				var count_items = e.items.length;
				...
				$.each(e.items, function(j, item) {
					...
					var addServiceToAssistants = function(
						service_start,
						service_end,
						classes,
						border_top,
						...
						var objs = $self.applyAdvancedFilters([
							{
								...
								deposit: e.deposit,
								due: e.due,
								status: count_items - 1 === k ? e.status : "",
								customer: e.customer,
								...
								calendar_day: k === 0 ?
									e.calendar_day_assistants[item.service] : e.calendar_day_title_assistants[item.service],
								url: e.url,
						...
						if(!Array.isArray(att_id)){
							if (typeof _assistants[att_id] === "undefined") {
									_assistants[att_id] = [];
							}
							_assistants[att_id].push(eventVal);
						}else{
							att_id.forEach(function(_id){
								if(typeof _assistants[_id] === 'undefined'){
									_assistants[_id] = [];
								}
								_assistants[_id].push(eventVal);
							});
						}
					};
				...
				_assistants.forEach(function(_events, _att_id) {
					var col = 0;

					if (!(_att_id in assistants)) {
						assistants[_att_id] = [];
						assistants[_att_id].eventCount = 0;
					} else {
						//first service of the booking for attendant
						var _event_start = _events[0][0];

						//last service of the booking for attendant
						var _event_end =
							_events[_events.length - 1][0] +
							_events[_events.length - 1][1];
						var checkifcontained = eventOverlap(
							_event_start,
							_event_end,
							assistants[_att_id]
						);
						if (checkifcontained) {
							assistants[_att_id].eventCount = Math.max(
								assistants[_att_id].eventCount,
								checkifcontained
							);
							col = checkifcontained;
						}
					}
					...
					_events.forEach(function(_event) {
						//set column
						_event[2] = col;
						assistants[_att_id].push(_event);
					});
				});
			});
			...
			attIds.forEach(function(att_id) {
				assistants[att_id].forEach(function(event) {
					...
					if(by_page.indexOf(obj) == -1){
						by_page.push(Object.assign({}, obj)); // cloning event for multiple assistents
					}else{
						by_page.push(obj);
					}
				});
				...
				if (assistants[att_id].eventCount)
					ret = ret.concat(
						Array.apply(
							null,
							Array(assistants[att_id].eventCount)
						).map(function() {
							return "undefined";
						})
					);
			...
			$.each(data.events, function(k, e) {
				...
				if (e._start < start.getTime()) {
					start = s;
					time_start = e.start_hour.split(' ')[2].split(':');
					e.start_hour = time_start;
					$self.options.time_start = e.start_hour.join(':');
					let time_diff = parseInt(time_end[0]) * 60 +
									parseInt(time_end[1]) -
									(parseInt(time_start[0]) * 60 + parseInt(time_start[1]));
					data.lines = time_diff / time_split;
				...
				if (e._end > end.getTime()) {
					...
					$self.options.time_end = e.end_hour.split(' ')[2];
					let time_diff = parseInt(time_end[0]) * 60 +
									parseInt(time_end[1]) -
									(parseInt(time_start[0]) * 60 + parseInt(time_start[1]));
					data.lines = time_diff / time_split;
				...
				$.each(e.items, function(j, item) {
					...
					var addServiceToEvent = function(
						service_start,
						service_end,
						classes,
						border_top
						...
						var objs = $self.applyAdvancedFilters([
							{
								...
								services: [item.service],
								class: e.class + (classes || ""),
								calendar_day: k === 0 ? e.calendar_day : "",
								url: e.url,
								delete_url: e.delete_url,
								duplicate_url: e.duplicate_url,
								_start: service_start,
				...
				$.each(event, function(j, obj) {
					...
					while (!added) {
						...
						if (grid[row_rounded][col] == undefined) {
							...
							for (
								var offset = 0;
								parseFloat(offset.toFixed(4)) < offset_max;
								offset += step
							) {
	...
	Calendar.prototype._update_day_prepare_sln_booking_editor = function() {
		...
		$(".events-list .event, .cal-cell1 .cal-event-week")
			.off("click")
			.on("click", function(e) {
				e.preventDefault();
				e.stopPropagation();
